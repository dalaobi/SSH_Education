...
//	get("fun2").style.display="block";
	get("fun3").style.display="block";
	var t = $("#blog"+id).children("div.blog-header").html();
	t = t.substring(0,t.indexOf("<"));
	var t1 = $("#blog"+id).children("div.blog-content").text();
	console.log(t);
	console.log(t1);
//	$("#fun3").children().children(".row").children(".span11").children().attr("value",t);//设置标题
//	var temp = $("#fun3").children().children(".row").toArray()[0].children(".row").children(".span11").children().attr("value",t);
	temp = $("#fun3").children().children(".row").toArray();
	ss = $(temp[2]).children().attr("onclick","updateblog("+id+")");
	sse = $(temp[1]).children("div.span11").children("div#mceu_102").children("div#mceu_102-body").children("div#mceu_125").children().contents().find("p").text(t1);
...
function responseTo1(){
	console.log("statecode:"+xmlHttp.readyState);
...
//					+'<span class="date">'+blogs[split_num*tempindex+i].date+'</span></div><div class="blog-content">'+blogs[split_num*tempindex+i].content+'</div></div>';
				...
				+'<a href="javascript:editblog('+blogs[calc].id+')" class="delete">编辑</a>'
...
//		var blogcount = get("blogcount");
		...
		console.log("blogs count:"+blog_num);
//		blogcount.innerHTML = sdfsdf;//"您当前共有"+blog_num+"份日志";
		$(".counter").text("您当前共有"+blog_num+"份日志");
		console.log("setfooter start...");
		setfooter();
		console.log("setfooter end...");
	}
...
function updateblog(id){
	...
	createXmlHttp();
	title = get("title1").value;
	content = $(temp[1]).children("div.span11").children("div#mceu_102").children("div#mceu_102-body").children("div#mceu_125").children().contents().find("p").text();
	console.log(title);
	console.log(content);	
	if(title.length<=40&&content.length<=800){
		xmlHttp.open("POST", "blog_update.action?"+"id="+id+"&title="+title+"&content="+content,true);	
...
function blog_up(){
	...
	content  = get("conup").value;
	
	if(title.length<=40&&content.length<=800){
		console.log("blog  upload...");
		console.log(title);
		console.log(content);
		xmlHttp.open("GET", "uploadblog.action?title="+title+"&content="+content,true);			
