	...
	if(xmlHttp.readyState==4){
		...
		for(var tempindex=0; tempindex<total; tempindex++){
			...
			for(var i=0;i<6;i++){
				...
				html += "<tr><td width=\"400px\"><a href=\"javascript:list_aem(\'"+enterprises[6*tempindex+i]+"\')\" class=\"ename\">"+enterprises[6*tempindex+i].split("@")[0]+"</a></td><td width=\"400px\"><a class=\"renum\">"+num[6*tempindex+i]+"个职位</a></td></tr>";
			}
			html += "</tbody>";
			html += "</table>";
			html+='</div>';
...
function responseTo2(){
	if(xmlHttp.readyState==4){
		...
		for(var tempindex=0; tempindex<total; tempindex++){
			...
			for(var i=0;i<6;i++){
				if(6*tempindex+i>=enterprises.length)break;	//此时说明所有图片都添加完毕
				html += "<tr><td width=\"400px\"><a href=\"javascript:list_nem(\'"+enterprises[6*tempindex+i]+"\')\" class=\"ename\">"+enterprises[6*tempindex+i].split("@")[0]+"</a></td><td width=\"400px\"><a class=\"renum\">"+num[6*tempindex+i]+"个职位</a></td><tr>";			
			}
			html += "</tbody>";
			html += "</table>";
			html+='</div>';
...
//调整页码和表格之间的距离
function adjustDis(number){
	var RJPageBar = document.getElementById("RJ-pagebar"+number);
