...
function getOtherEn(){
	if(xmlHttp.readyState==4){
		if(xmlHttp.status == 200){
			...
			html+='<div id="plist'+1+'">';	//每页的id为“plist+页码”
			html += "<table id=\"etable1\" border=\"1\"     cellspacing=\"0\"    bordercolor=\"#BBBBBB\">";
			html += "<tbody id=\"tbody1\">";
			for(var i = 0; i<resultList.length; i++){
				html += '<tr><td><a class="title" href="enterprise.action?eid='+resultList[i].id+'">'+resultList[i].name+'</a></td>'
					+'<td><a class="top" href="javascript:setTopEnterprise('+resultList[i].id+')">置顶</a></td>';
				if(!resultList[i].recommend)
					html += '<td><a class="top" href="javascript:setRecEnterprise('+resultList[i].id+')">推荐</a></td></tr>';
				else
					html += '<td><a class="top" href="javascript:unSetRecEnterprise('+resultList[i].id+')">取消推荐</a></td></tr>';
			}
			html += "</tbody>";
			html += "</table>";
			html +='</div>';
