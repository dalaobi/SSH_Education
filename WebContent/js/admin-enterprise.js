...
function responseTo1(){
		...
		if(xmlHttp.readyState==4){
			...
			for(var tempindex=0; tempindex<total; tempindex++){
				...
				html += "<tr>";
				html += "<td id=\"vhname\">公司名称</th>";
				html += "<td id=\"vhaddress\">公司地址</th>";
				html += "<td colspan = 3 id=\"vhmeterial\">相关材料</th>";
				html += "</tr>";
				...
				for(var i=0;i<=5;i++){
					...
					html+="<tr id=\""+enterprises[calc].id+"\" class=\"enclause\"><td class=\"column1\" ><a href=\"enterprise.action?eid="+enterprises[calc].id+"\">"+enterprises[calc].name+"</a></td><td class=\"column2\" ><a>"+enterprises[calc].address+"</a></td><td class=\"column3\" ><a href=\""+enterprises[calc].material+"\">下载</a></td><td class=\"column5\" ><a href=\"javascript:del("+enterprises[calc].id+")\">删除</a></td></tr>"
...
function responseTo2(){
	...
	if(xmlHttp.readyState==4){
		...
		var obj = eval('('+response+')');
		var enterprises = obj.result;
		var enlist = get("elist2");
		total = Math.ceil(enterprises.length/6);		//计算分页的总页数
		var html = "";
		enlist.innerHTML="";
		var calc = 0;
		var size = enterprises.length;
		html+='<div id="plist'+2+'">';	//每页的id为“plist+页码”
		
		html += "<table id=\"etable2\" border=\"1\"     cellspacing=\"0\"    bordercolor=\"#BBBBBB\">";
		...
		for(var i=0; i<enterprises.length; i++){
			...
			html+="<tr id=\""+enterprises[i].id+"\" class=\"enclause\"><td class=\"column1\" ><a>"+enterprises[i].name+"</a></td><td class=\"column2\" ><a>"+enterprises[i].address+"</a></td><td class=\"column3\" ><a href=\""+enterprises[i].material+"\">下载</a></td><td class=\"column4\" ><a href=\"javascript:adm("+enterprises[i].id+")\">通过</a></td><td class=\"column5\" ><a href=\"javascript:del("+enterprises[i].id+")\">拒绝</a></td></tr>";
				//table.innerHTML+=html;
