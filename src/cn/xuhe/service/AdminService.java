...
public class AdminService {
	...
	public boolean savePassword(String adminid,String password){
	...
	public void delete_enterprise(int id){//删除公司，以及公司发布的招聘，如果有需要，可以在此删除投到该公司的简历
		Enterprise enterprise = enterpriseDAO.findById(id);
		if(enterprise==null)
			...
			List<Employment> emlist = employmentDAO.findByEnterpriseid(id);
			int size = emlist.size();
			if(size > 0){
				for(int i=0;i<size;i++){
					employmentDAO.delete(emlist.get(i));
				}
	...
	public void delete_school(String id){
		...
		if(school==null)
			...
			if(size > 0){
				for(int i=0;i<size;i++){
					if(stulist.get(i).getStudentid().split("@")[1].equals(id)){
						studentDAO.delete(stulist.get(i));
					}
				}
			}
		}
