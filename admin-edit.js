	...
	if (xmlHttp.readyState == 4) {
		if (xmlHttp.status == 200) {
			...
			for ( var i = 0; i < resultList.length; i++) {
				html += '<a class="title" href="enterprise.action?eid='
						+ resultList[i].id + '">' + resultList[i].name + '</a>'
						+ '<a class="top" href="javascript:setTopEnterprise('
						+ resultList[i].id + ')">置顶</a>';
...
function searchIn() {
	...
	function webChange() {
		...
		if (searchValue) {
			...
			$(document).ready(function() {
				// 公司的個數
					var size = $("div#newslist").children("a.title").length;
					$index = $("div#newslist").children().first();
					for ( var i = 0; i < size; i++) {
						console.log($index.text());
						console.log($index.text().match(searchValue));
						if (!$index.text().match(searchValue)) {
							$index.hide();
							$index = $index.next();
							$index.hide();
							$index = $index.next();
		...
		// var news = get("newslist").innerHTML;
		// console.log(news);
		// console.log('sdfsdfds');
		// console.log(mytext.parentElement);
		// console.log(mytext.nextSibling);
...
function responseTo1() {
	if (xmlHttp.readyState == 4) {
		...
		for ( var i = informations.length; i > 0; i--) {
			html += "<a class=\"title\" href=\"javascript:neededit("
					+ informations[i - 1].id
					+ ")\">"
					+ informations[i - 1].title.split(";")[0]
					+ "&nbsp&nbsp&nbsp&nbsp"
					+ informations[i - 1].date
					+ "</a><a class=\"top\" href=\"infoup_top.action?id="
					+ informations[i - 1].id
					+ "&type="
					+ informations[i - 1].type
					+ "\">置顶</a><a class=\"del\" href=\"infoup_delete.action?id="
					+ informations[i - 1].id + "&type="
					+ informations[i - 1].type + "\">删除</a>";
		}
		get("newslist").innerHTML = html;
	}
}
...
function responseTo4() {
	if (xmlHttp.readyState == 4) {
		...
		for ( var i = 0; i < informations.length; i++) {
			html += '<a class="title" href="enterprise.action?eid='
					+ informations[i].id + '">' + informations[i].name + '</a>'
					+ '<a class="top" href="javascript:setTopEnterprise('
					+ informations[i].id + ')">置顶</a>';
...
function doing() {
	...
	inspector = setInterval(function() {
		var status_text = get("uploadStatus").contentWindow.document
							...
							function() {
								get("conup")
										.setAttribute(
												"value",
												document
														.getElementsByTagName("iframe")[1].contentWindow.document
														.getElementsByTagName("body")[0].innerHTML)
							}, 1000)
